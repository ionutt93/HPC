...
*/
float timestep(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles); //made void ------------------------------------
...
int main(int argc, char* argv[])
{
  ...
  for (ii=0;ii<params.maxIters;ii++) {
    av_vels[ii] = timestep(params,cells,tmp_cells,obstacles);
...
float collision(const t_param params, t_speed* cells, t_speed* tmp_cells, int* obstacles)
{
  int ii,jj,kk, tot_cells = 0;                 /* generic counters */
  ...
  float u_sq, tot_u = 0.0;                  /* squared velocity */
  ...
  ** are in the scratch-space grid */
  ...
  #pragma omp parallel for private(u_x, u_y, u, d_equ, u_sq, local_density, jj, ii, kk) firstprivate(obstacles) reduction(+:tot_cells) reduction(+:tot_u) shared(tmp_cells, cells) default(none)
  for(ii=0;ii<params.ny;ii++) {
    for(jj=0;jj<params.nx;jj++) {
	...
	/* axis speeds: weight w1 */
       //--------------------------------------------------------------
       // 1.0 + (u[1] * (2 * c_sq + u[1]) - u_sq * c_sq)/(2 * c_sq * c_sq)
       d_equ[1] = w1 * local_density * (1.0 + (u[1] * (2 * c_sq + u[1]) - u_sq * c_sq) / (2 * c_sq * c_sq));
       d_equ[2] = w1 * local_density * (1.0 + (u[2] * (2 * c_sq + u[2]) - u_sq * c_sq) / (2 * c_sq * c_sq));
       d_equ[3] = w1 * local_density * (1.0 + (u[3] * (2 * c_sq + u[3]) - u_sq * c_sq) / (2 * c_sq * c_sq));
       d_equ[4] = w1 * local_density * (1.0 + (u[4] * (2 * c_sq + u[4]) - u_sq * c_sq) / (2 * c_sq * c_sq));
  /* diagonal speeds: weight w2 */
       d_equ[5] = w2 * local_density * (1.0 + (u[5] * (2 * c_sq + u[5]) - u_sq * c_sq) / (2 * c_sq * c_sq));
       d_equ[6] = w2 * local_density * (1.0 + (u[6] * (2 * c_sq + u[6]) - u_sq * c_sq) / (2 * c_sq * c_sq));
       d_equ[7] = w2 * local_density * (1.0 + (u[7] * (2 * c_sq + u[7]) - u_sq * c_sq) / (2 * c_sq * c_sq));
       d_equ[8] = w2 * local_density * (1.0 + (u[8] * (2 * c_sq + u[8]) - u_sq * c_sq) / (2 * c_sq * c_sq));

       // Changed the formula to this equation | not much changed when added -ffast-math -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
       // d_equ[1] = w1 * local_density * (1.0 + 3.0 * u[1] + 4.5 * u[1] * u[1] - 1.5 * u_sq);
       // d_equ[2] = w1 * local_density * (1.0 + 3.0 * u[2] + 4.5 * u[2] * u[2] - 1.5 * u_sq);
       // d_equ[3] = w1 * local_density * (1.0 + 3.0 * u[3] + 4.5 * u[3] * u[3] - 1.5 * u_sq);
       // d_equ[4] = w1 * local_density * (1.0 + 3.0 * u[4] + 4.5 * u[4] * u[4] - 1.5 * u_sq);

       // d_equ[5] = w2 * local_density * (1.0 + 3.0 * u[5] + 4.5 * u[5] * u[5] - 1.5 * u_sq);
       // d_equ[6] = w2 * local_density * (1.0 + 3.0 * u[6] + 4.5 * u[6] * u[6] - 1.5 * u_sq);
       // d_equ[7] = w2 * local_density * (1.0 + 3.0 * u[7] + 4.5 * u[7] * u[7] - 1.5 * u_sq);
       // d_equ[8] = w2 * local_density * (1.0 + 3.0 * u[8] + 4.5 * u[8] * u[8] - 1.5 * u_sq);

	/* relaxation step */
       for(kk=0;kk<NSPEEDS;kk++) {
         cells[ii*params.nx + jj].speeds[kk] = (tmp_cells[ii*params.nx + jj].speeds[kk] + params.omega * (d_equ[kk] - tmp_cells[ii*params.nx + jj].speeds[kk]));
       }
     }
   }
 }
